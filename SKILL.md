...
For u4 weights -> bf16 output GEMV (`y = A*x`, `4096x4096`, bf16 A, u4-packed x, per-group f16 scales, zero-point 8): naive 2.22 ms -> host-dequant x to bf16 once + one row per sub-group with `vec<bf16,16>` loads and float accumulators 0.0883 ms, vs oneDNN GPU matmul `Kx1` 0.1456 ms (`jit:gemm:any`). The same 1D `nd_range` rule as the f32 GEMV applies: derive per-lane trip counts from the actual sub-group size, or A770's 32-lane default silently computes half the rows. Details and copy-ready kernel: [techniques.md](references/techniques.md#u4-bf16-gemv-ladder-4096x4096-bf16-a-u4-x-group_size128), [code-snippets.md](references/code-snippets.md#u4-bf16-gemv-core-sub-group-per-row-bf16-vector--float-accumulator).
...
- u4 -> bf16 GEMV: host-dequantize the u4 vector to bf16 once, then run the standard bf16 GEMV kernel with a float accumulator. The oneDNN baseline is `Kx1` with u4 `ba` weights, f16 group scales, zero-point 8, and `fpmath_mode::any`; f16 and bf16 src both select `jit:gemm:any`.
...
- For u4 -> bf16 GEMV, host-dequantize the u4 vector to bf16 once before timing; re-unpacking u4 and scales per row keeps the kernel instruction-bound. The stable SYCL fast path is the bf16 GEMV kernel with a float accumulator, not a mixed u4/bf16 DPAS.
- Do not accumulate bf16 products in a bf16 `vec` accumulator for large K. At `4096x4096` it fails 535/4096 rows even though it passes at `64x128`; keep the vector accumulator in `float` and round only the final row sum.
- Manual 2/4-way vector unrolling of the bf16 GEMV was negative: `0.124` ms for both unroll variants vs `0.088` ms for the simple compiler-scheduled loop.
- oneDNN u4 `Kx1` with `fpmath_mode::any` can differ from the f32/bf16 reference at K=4096 (39/4096 rows at 5% relative tolerance). Report its timing as a baseline but gate correctness on the SYCL reference.
